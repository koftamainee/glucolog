const Storage = (() => {
  ...
  function addGlucosePoint(dateKey, hour, value) {
    const db = loadAll();
    if (!db[dateKey]) db[dateKey] = {};
    if (!db[dateKey].glucose) db[dateKey].glucose = [];
    const idx = db[dateKey].glucose.findIndex(p => p.h === hour);
    if (idx >= 0) {
      db[dateKey].glucose[idx].g = value;
    } else {
      db[dateKey].glucose.push({ h: hour, g: value });
  ...
  return { getDay, setField, setMealField, addGlucosePoint, toggleArrayItem };
})();
