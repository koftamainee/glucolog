const Sidebar = (() => {
    ...
    function build() {
        ...
        drawer.querySelector('.sidebar-close').addEventListener('click', close);

        drawer.querySelector('#sidebarExport').addEventListener('click', doExport);
        drawer.querySelector('#sidebarImport').addEventListener('click', () => {
            drawer.querySelector('#sidebarFileInput').click();
        });

        drawer.querySelector('#sidebarFileInput').addEventListener('change', function () {
    ...
    function showStatus(msg, ok) {
        ...
        clearTimeout(el._t);
        el._t = setTimeout(() => { el.textContent = ''; el.className = 'sidebar-import-status'; }, 3000);
    }
