    ...
    private void hasAvailableDevices(Context context, BaseApi.Callback<Node> callback) {
        ...
        nodeApi.getConnectedNodes(new BaseApi.Callback<List<Node>>() {
            ...
            public void onSuccess(List<Node> obj) {
                ...
                android.util.Log.w("XiaomiWatchHelper", "getConnectedNodes count=" + obj.size());
                for (Node each : obj) {
                    android.util.Log.w("XiaomiWatchHelper", "node id=" + each.getId() + " name=" + each.getName());
                    callback.onSuccess(each);
                }
                if (obj.isEmpty()) {
                    callback.onFailure(Status.RESULT_DISCONNECTED);
                }
            }

            @Override
            public void onFailure(Status status) {
                android.util.Log.w("XiaomiWatchHelper", "getConnectedNodes onFailure code=" + status.getCode());
    ...
    private void getIsConnected(Node device, BaseApi.Callback<Node> callback) {
        ...
        nodeApi.query(device.getId(), DataItem.ITEM_CONNECTION, new BaseApi.Callback<DataQueryResult>() {
            ...
            public void onSuccess(DataQueryResult obj) {
                android.util.Log.w("XiaomiWatchHelper", "getIsConnected onSuccess device=" + device.getId() + " connected=" + (obj != null && obj.isConnected));
            ...
            @Override
            public void onFailure(Status status) {
                android.util.Log.w("XiaomiWatchHelper", "getIsConnected onFailure code=" + status.getCode());
                callback.onFailure(status);
            }
        });
    }
    private void checkPermission(Node device, BaseApi.Callback<Boolean> callback) {
        ...
        authApi.checkPermissions(device.getId(), permissions, new BaseApi.Callback<Boolean[]>() {
            ...
            public void onSuccess(Boolean[] obj) {
                ...
                android.util.Log.w("XiaomiWatchHelper", "checkPermission granted=" + grant);
                if (grant) {
                    ...
                    requestPermission(device, permissions, new BaseApi.Callback<Permission[]>() {
                        ...
                        public void onSuccess(Permission[] obj) {
                            android.util.Log.w("XiaomiWatchHelper", "requestPermission onSuccess count=" + obj.length);
                            callback.onSuccess(true);
                        }

                        @Override
                        public void onFailure(Status status) {
                            android.util.Log.w("XiaomiWatchHelper", "requestPermission onFailure code=" + status.getCode());
                            callback.onFailure(status);
                        }
                    });
                }
            ...
            @Override
            public void onFailure(Status status) {
                android.util.Log.w("XiaomiWatchHelper", "checkPermission onFailure code=" + status.getCode());
                callback.onFailure(status);
            }
        });
    }
