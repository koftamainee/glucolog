...
- **Глюкоза импортируется полностью** (Этап D, смена решения): и xdrip, и ручные
  показания с сохранением `source`; дедуп по уникальному `(date, h, source)`
  (уже существующее значение обновляется, остальное вставляется). Строки `Глюкоза`
  в CSV и массив `glucose` в JSON разбираются; для старого формата без источника
...
- **Заменить**: `deleteAll` по `glucose` (если в файле есть глюкоза) и
  `insulin/meal/day/stool`, затем запись. Если файл старого формата без глюкозы,
  `glucose` не трогается (живые данные xDrip сохраняются).
- **Объединить**: поле-уровневый merge — существующая строка обновляется только по
  пришедшим из файла полям (глюкоза по `(date, h, source)`, инсулин по `(date, h)`,
  блюда по `(date, key)`, `day` — copy с `?:` текущими), стул — `insert(IGNORE)`.
  Новые дни добавляются.
- Диалог выбора стратегии показывается только если в БД уже есть данные
  (`hasData`, включая глюкозу); иначе импорт сразу.
